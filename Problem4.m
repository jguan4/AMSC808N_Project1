...
for i = 1:size(results_plot,2)
    legend_name = sprintf('M = %d',results_plot(i).M);
    ks = 500:600;
    plot(ks,results_plot(i).tss_ave(ks),'LineWidth',2,'DisplayName',legend_name);
    hold on;
end
xlabel('Iteration','Fontsize',fsz)
ylabel('runtime per iteration (s)','Fontsize',fsz);
lgd = legend;
lgd.FontSize = fsz;
grid on
pbaspect([1.3 1 1])
...
a=figure;
plot(1,sum(results_plot_sl(1).tss_ave),'bo','LineWidth',2,'DisplayName','SLBFGS');
hold on;
plot(2,sum(results_plot_sg(1).tss_ave),'bo','LineWidth',2,'DisplayName','SG');
plot(3,sum(results_plot_si(1).tss_ave),'bo','LineWidth',2,'DisplayName','SINewton');
xticks([1,2,3])
